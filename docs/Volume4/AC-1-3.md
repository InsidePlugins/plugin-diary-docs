#  AC-1-3 调试 HarmonyAuth SMART Part I
...
实际上，还有另外一个问题，那就是玩家可以丢弃物品。

如果有人逮到了一个创造模式的玩家，那这个人直接就发财了，因为玩家可以从创造背包取出物品，然后丢弃，这样就可以无限制地取得物品。
...
打开Minecraft，进入服务器，发送`/hl`。

发生什么了？

恭喜！这个账号再也无法登陆啦！看下面的图片！Successful Hacking Attempt！（大雾）

![BUG-1.png](https://s2.loli.net/2023/07/24/QmsqdDoGT4r7z6i.png)

相信你已经从控制台上看出一些东西来了，我们没有检查参数数量。

在`onLoginCommand`里加上下面代码，就好了。
```java
if (args.length < 1) {
    return false;
}
...
通过观察Minecraft服务端的标准输入和输出，我们成功的解决了一些漏洞。

但是我们的插件还仍然有许多漏洞，在下一章节，我们将利用「IDEA」的调试功能，完成一些复杂的调试。